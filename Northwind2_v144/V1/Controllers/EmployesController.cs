...
using Northwind2_v144.V1.Entities;
...
namespace Northwind2_v144.V1.Controllers
{
	...
	public class EmployesController : ControllerBase
	{
		 ...
		 GetEmployés([FromQuery] string? rechercheNom, [FromQuery] DateTime? dateEmbaucheMax)
	 {
		...
		/// </summary>
		/// <param name="empDTO">Employé à créer</param>
		...
		public async Task<ActionResult<Employe>> PostEmployé(EmployeDTO empDTO)
		{
			try
			{
				// Mappe l'entité DTO sur l'entité du modèle EF
				Employe emp = new()
				{
					IdAdresse = empDTO.IdAdresse,
					IdManager = empDTO.IdManager,
					Nom = empDTO.Nom,
					Prenom = empDTO.Prenom,
					Fonction = string.Empty,
					Civilite = empDTO.Civilite,
					DateNaissance = empDTO.DateNaissance,
					DateEmbauche = empDTO.DateEmbauche
				};
		...
		[HttpPost("formdata")]
		[Authorize(Policy = "GérerEmployés")]
		public async Task<ActionResult<Employe>> PostEmployéFormData([FromForm] FormEmployeDTO fe)
		{
			Employe emp = new()
			{
